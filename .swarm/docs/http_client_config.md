...
- **formData branch commented out:** Only the `convertHttpRequest` branch in `RequestClient.js` has its encoding logic commented out (`// options.formData = APIHelper.formDataEncodeObject(req.formData);`). `executeRequest` still has a separate, active formData path (`rq.form()` + `appendFormdata`, which uses `APIHelper.formDataEncodeObject` and `form.append`), so if `req.formData` is set the data is attached to the outgoing request. No current controller uses formData, so this path is not exercised in practice.
