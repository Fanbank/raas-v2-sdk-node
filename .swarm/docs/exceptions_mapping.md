...
- **Null handling in mapFields:** If a JSON field is null or undefined and the model instance has a default value (e.g., `currencyCode: 'USD'` in `AccountModel`), the instance default is preserved. For non-defaulted fields, null/undefined JSON values are passed through as-is.
